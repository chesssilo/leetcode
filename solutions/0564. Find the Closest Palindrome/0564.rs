...
impl Solution {
    ...
    fn get_palindromes(s: &str) -> (i64, i64) {
      ...
      let half = &s[0..(sz + 1) / 2];
      let reversed_half: String = half.chars()
        .take(sz / 2)
        .collect::<Vec<_>>()
        .into_iter()
        .rev()
        .collect();
      let candidate = format!("{}{}", half, reversed_half)
        .parse::<i64>()
        .unwrap();
      ...
      if candidate < num {
        ...
        let reversed_prev_half: String = prev_half.chars()
          .take(sz / 2)
          .collect::<Vec<_>>()
          .into_iter()
          .rev()
        ...
        if sz % 2 == 0 && prev_half.parse::<i64>().unwrap() == 0 {
          ...
          palindromes.push(format!("{}9{}", prev_half, reversed_prev_half)
            .parse::<i64>()
            .unwrap());
        } else {
          palindromes.push(format!("{}{}", prev_half, reversed_prev_half)
            .parse::<i64>()
            .unwrap());
        }
      }
      ...
      if candidate > num {
        ...
        let reversed_next_half: String = next_half.chars()
          .take(sz / 2)
          .collect::<Vec<_>>()
          .into_iter()
          .rev()
          .collect();
        palindromes.push(format!("{}{}", next_half, reversed_next_half)
          .parse::<i64>()
          .unwrap());
      }
