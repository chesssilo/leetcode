...
impl Solution {
  ...
  fn postorder(tree: &Vec<HashSet<usize>>, node: usize, 
                parent: i32, count: &mut Vec<i32>, ans: &mut Vec<i32>) {
  ...
  fn preorder(tree: &Vec<HashSet<usize>>, node: usize, 
              parent: i32, count: &mut Vec<i32>, ans: &mut Vec<i32>) {
    for &child in &tree[node] {
      if child as i32 == parent {
        continue;
