...
impl Solution {
  pub fn add_one_row(mut root: Option<Node>, val: i32, depth: i32) -> Option<Node> {
    if depth == 1 {
      let t = TreeNode{left: root, ..TreeNode::new(val)};
    ...
    fn add_row(node: Option<&mut Node>, val: i32, k: i32, depth: i32) {
      match node {
        Some(node) => {
          ...
          if k == depth - 1 {
            node.left = Some(Rc::new(RefCell::new(TreeNode{left: node.left.take(), 
                            ..TreeNode::new(val)})));
            node.right = Some(Rc::new(RefCell::new(TreeNode{right: node.right.take(),
                           ..TreeNode::new(val)})));
            return
          } else {
