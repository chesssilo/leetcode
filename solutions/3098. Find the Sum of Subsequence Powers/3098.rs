...
impl Solution {
  pub fn sum_of_powers(mut nums: Vec<i32>, k: i32) -> i32 {
    ...
    fn dp(
      ...
      if let Some(&result) = memo.get(
        &(i, k, last_picked_index, first_index, second_index)
      ) {
        return result;
      }
      ...
      let pick = dp(
        i + 1, k - 1, i as isize, new_first_index, new_second_index, nums, memo
      );
      let skip = dp(
        i + 1, k, last_picked_index, first_index, second_index, nums, memo
      );
      let result = (pick + skip) % KMOD;

      memo.insert(
        (i, k, last_picked_index, first_index, second_index), result
      );
      result
    }
