...
impl Solution {
  pub fn get_results(queries: Vec<Vec<i32>>) -> Vec<bool> {
    ...
    for query in queries.iter().rev() {
      ...
      if query[0] == 1 {
        if pos + 1 < blocks.len() {
          Self::update_segment_tree(&mut segment_tree, 
        ...
        let can_place = query[1] - blocks[pos - 1] >= query[2] || 
                        Self::query_segment_tree(&segment_tree, 
                                                  query[1] as isize) >= 
                                                  query[2];
        ans.push(can_place);
      }
  ...
  fn update_segment_tree(segment_tree: &mut Vec<i32>, 
                          mut pos: usize, value: i32) {
  ...
  fn update_blocks(blocks: &mut Vec<i32>, segment_tree: &mut Vec<i32>) {
    for i in 1..blocks.len() {
      Self::update_segment_tree(segment_tree, 
                                blocks[i] as usize, 
                                blocks[i] - blocks[i - 1]);
    }
  }
