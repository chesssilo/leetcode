...
impl Solution {
  pub fn replace_value_in_tree(root: TreeNodeRef) -> TreeNodeRef {
    ...
    fn replace(
      ...
      if let Some(n) = node {
        ...
        if let Some(left) = &n.left {
          let new_left = 
            Rc::new(RefCell::new(TreeNode::new(next_level_cousins_sum)));
          curr.as_ref().unwrap().borrow_mut().left = Some(new_left.clone());
          replace(
            &Some(left.clone()), next_level, &Some(new_left), level_sums
          );
        }
        if let Some(right) = &n.right {
          ...
          replace(
            &Some(right.clone()), next_level, &Some(new_right), level_sums
          );
        }
      }
      curr.clone()
    ...
    replace(
      &root, 0, &Some(Rc::new(RefCell::new(TreeNode::new(0)))), &level_sums
    )
  }
}