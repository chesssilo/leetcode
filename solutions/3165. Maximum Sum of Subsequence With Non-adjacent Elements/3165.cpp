...
class Solution {
 public:
  int maximumSumSubsequence(vector<int>& nums, 
    ...
    for (const auto &query : queries) {
      ...
      parts[partIndex] = calculateMaxNonAdjacentSum(nums, 
                                                    partIndex * partSize,
 ...
 private:
  ...
  array<int, 4> calculateMaxNonAdjacentSum(vector<int>& nums, 
                                            int start, int end) {
    int firstLast = 0;
    ...
    for (int i = start; i < min(static_cast<int>(nums.size()), end); ++i) {
      firstLast = max(exchange(firstSecondLast, firstLast) + nums[i], 
                               firstLast);
      if (i > start)
        secondLast = max(exchange(secondSecondLast, secondLast) + nums[i], 
                                  secondLast);
    }
  ...
  int combineParts(vector<array<int, 4>>& parts) {
    ...
    for (auto [firstLast, firstSecondLast, 
               secondLast, secondSecondLast] : parts) {
      int temp = maxLast;
      maxLast = max(maxLast + secondLast, maxSecondLast + firstLast);
      maxSecondLast = max(temp + secondSecondLast, 
                          maxSecondLast + firstSecondLast);
    }
