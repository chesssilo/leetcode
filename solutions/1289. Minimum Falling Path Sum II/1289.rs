...
impl Solution {
  pub fn min_falling_path_sum(mut grid: Vec<Vec<i32>>) -> i32 {
    ...
    for i in 1..n {
      let two_min_num_and_indices = 
        Self::get_two_min_num_and_indices(&grid[i - 1]);
  ...
  fn get_two_min_num_and_indices(nums: &[i32]) -> Vec<(i32, usize)> {
    let mut num_and_indices: Vec<(i32, usize)> = nums.iter()
                                                     .enumerate()
                                                     .map(|(i, &num)| (num, i))
                                                     .collect();
    num_and_indices.sort_unstable_by(|a, b| a.0.partial_cmp(&b.0)
