...
impl Solution {
  pub fn create_binary_tree(
    ...
    for d in descriptions.iter() {
      ...
      let parent = val_to_node.entry(p).or_insert_with(|| {
        Rc::new(RefCell::new(TreeNode::new(p)))
      }).clone();
      let child = val_to_node.entry(c).or_insert_with(|| {
        Rc::new(RefCell::new(TreeNode::new(c)))
      }).clone();
      if is_left {
    ...
    let root_val = val_to_node.keys().find(|&&k| {
      !children.contains(&k)
    }).unwrap();
